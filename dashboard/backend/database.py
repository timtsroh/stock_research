...
class MacroPanel(Base):
    ...
    id     = Column(Integer, primary_key=True, index=True)
    slot   = Column(Integer, unique=True)     # 0~5
...
def init_db():
    ...
    if db.query(MacroPanel).count() == 0:
        panels = [
            MacroPanel(slot=0, ticker="^TNX",     label="미국 10Y 금리"),
            MacroPanel(slot=1, ticker="^GSPC",    label="S&P 500"),
            ...
            MacroPanel(slot=5, ticker="^NDX",     label="나스닥 100"),
        ]
        db.add_all(panels)
