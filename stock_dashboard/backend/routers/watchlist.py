...
def get_watchlist(db: Session = Depends(get_db)):
    ...
    result = []
    for item in items:
        info = fetch_stock_info(item.ticker, item.market)
        result.append(info)
    return result
...
def add_stock(req: AddStockRequest, db: Session = Depends(get_db)):
    existing = db.query(WatchlistItem).filter(WatchlistItem.ticker == req.ticker).first()
    if existing:
        raise HTTPException(status_code=400, detail="Already in watchlist")
    info = fetch_stock_info(req.ticker, req.market)
    item = WatchlistItem(ticker=req.ticker,
                         name=info.get("name", req.ticker),
                         market=req.market)
